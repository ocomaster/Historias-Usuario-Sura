...
<#if (bgCard.colorBgCard.getData())??>
	...
	<#assign bgColor = colorClass(bgColor, "", "") />
<#else>
...
<section class="s-c-card-tag-img ">
    <div  class="container"  >
		    ...
		    <!-----Declaracion de Border Radius en 12px, y contorno en 1px por Defecto   color: var(--second-color-3)  ---->
<div class="card-tag-img  color: bg-color${bgColor}"  style="border-color: var(-${selectColorBorderValue}); " >
        <!------------------divisor tag------------------->
            <#if haveCardTagText>
